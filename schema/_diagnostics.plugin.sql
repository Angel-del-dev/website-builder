...
CREATE TABLE DIAGNOSTIC_TRACE(
    SEED INT,
    ID INT,
    TRACE TEXT,
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT PK_DIAGNOSTIC_TRACE PRIMARY KEY(SEED, ID),
    CONSTRAINT FK_DIAGNOSTIC_TRACE FOREIGN KEY(SEED) REFERENCES DIAGNOSTICS_SEEDS(ID) ON DELETE CASCADE ON UPDATE CASCADE
);